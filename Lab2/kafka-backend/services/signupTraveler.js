...
function handle_request(body, callback){
    ...
    console.log("Inside new Traveler creation request");
    console.log(body);

     //Checking whether the user already exists
    Traveler.find({email: body.email}, function(err, data){
        if(err){
        throw err
                // callback(null, err)
        // console.log(reply);
    ...
    var Trav = new Traveler({firstname: body.firstname, lastname: body.lastname, email: body.email, password: body.password, travelerid: body.id})
    Trav.save(function(err, data){
        if(err){
        throw err
                callback(null, err)
        // console.log(reply);
